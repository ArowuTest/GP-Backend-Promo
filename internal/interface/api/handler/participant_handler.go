...
func (h *ParticipantHandler) ListUploadAudits(c *gin.Context) {
	...
	// Get audit repository from the application service
	auditRepository := h.uploadParticipantsService.GetAuditRepository()
	...
	// List upload audits directly from repository
	audits, err := auditRepository.ListUploadAudits(c.Request.Context(), page, pageSize)
	if err != nil {
		c.JSON(http.StatusInternalServerError, response.ErrorResponse{
			Success: false,
